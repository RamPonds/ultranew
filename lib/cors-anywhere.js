...
function withCORS(headers, request) {
  ...
  //headers['origin']=headers['origin'].replace("https://","").replace("http://","") || '';
...
console.log("new before proxy::::" +  req.headers.origin);

  req.headers.origin=req.headers.origin.replace("https://","").replace("http://","");
req.headers.origin = "abc.zohoapis.ramprasanth.com";
console.log("new after proxy::::" +  req.headers.origin);
  var location = req.corsAnywhereRequestState.location;
  req.url = location.path;
  //req.header['origin']=req.header['origin'].replace("https://","").replace("http://","");
...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    var origin = (req.headers.origin).replace("https://","").replace("http://","") || '';
